...
import { Ticket } from ".";
import { TicketPage, TicketsPage } from "../../../Pages";
...
test("Should remove the ticket from the list when the delete button is clicked", async () => {
    ...
    await expect(
        screen.queryByText("Urgent Fix Needed")
...
test("Should load the ticket data into the form inputs and display 'Edit ticket' button", async () => {
    ...
    const priorityInput = screen.getByPlaceholderText(
        "Priority"
    ) as HTMLInputElement;
...
test("Should edit the ticket with the changes made when clicking the edit button", async () => {
    ...
    await expect(
        screen.getAllByText(/Create framework/i)[0]
    ).toBeInTheDocument();
    await expect(
        screen.getAllByText(/Create framework/i)[1]
    ).toBeInTheDocument();
